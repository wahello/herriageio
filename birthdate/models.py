...
class BirthdateProfile(models.Model):
    ...
    reminder_frequencies = models.CharField(
        default="[1, 5, 30]", max_length=248)
...
class BirthdateContact(models.Model):
    def save(self, *args, **kwargs):
        if self.first_name is None and self.email is None and self.nickname is None:
            # cancel the save
            raise Exception(
                'All contacts must have at least one form of name identification.')
            return
        else:
    ...
    profile = models.ForeignKey(
        BirthdateProfile, null=False, blank=False, on_delete=models.CASCADE)
    ...
    def upcoming_birthday(self):
        ...
        todays_date = parse(str(date.today().month) + "/" +
                            str(date.today().day) + "/" + str(date.today().year)).date()
        upcoming_date = parse(str(upcoming_month) + "/" +
                              str(upcoming_day) + "/" + str(datetime.today().year)).date()
...
def create_profile(sender, instance, **kwargs):
    if settings.DEBUG:
        if instance.profile is None:
            instance.profile = BirthdateProfile.objects.create(
                id=int(Birthdate.profile.objects.count() + 1))
        ...
        while i < 5:
            self.contacts.add(BirthdateContact.objects.create(
                profile=BirthdateProfile.objects.create(
                    id=int(BirthdateProfile.objects.count() + 1)),
                first_name=" ".join(
                    ["%s" % (random.choice(first_names)) for _ in range(1)]),
                birthday=parse(str(str(random.randint(
                    1, 12)) + "/" + str(random.randint(1, 28)) + "/" + str(random.randint(1900, 2018))))
            ))
            i += 1
        self.save()
...
class BirthdateMessage(models.Model):
    template = models.ForeignKey(
        BirthdateMessageTemplate, null=True, blank=True, on_delete=models.SET_NULL)
    profile = models.ForeignKey(
        BirthdateProfile, null=False, blank=False, on_delete=models.CASCADE)
    contact = models.ForeignKey(
        BirthdateContact, null=False, blank=False, on_delete=models.CASCADE)
    ...
    def send_message(self):
        print("Dear %s, \n%s\n\n%s" %
              contact.first_name, template.subject, template.body)
        self.sent = True
    ...
    if created or not instance.reminders_scheduled:
        if instance.contact.reminder_status_str == 'enabled':
            for remind_days_before in instance.profile.get_reminder_frequencies():
                scheduled_for = instance.contact.upcoming_birthday - \
...
class BirthdateMessageReminderManager(models.Manager):
    ...
    def update_segment_reminder_statuses(self, segment_public_key, status_str):
        segment = BirthdateContactsSegment.objects.get(
            public_key=segment_public_key)
        ...
        if status_str == 'enabled':
            ...
            for contact in segment.contacts.all():
                if not contact.reminders_disabled_manually:
                    self.update_contact_reminder_statuses(
                        contact, status_str, segment_called=True)
            ...
            raise Exception(
                'A valid reminder status_str must be given to change the status.', status_str)
...
class BirthdateMessageReminder(models.Model):
    contact = models.ForeignKey(
        BirthdateContact, blank=False, null=False, on_delete=models.CASCADE)
